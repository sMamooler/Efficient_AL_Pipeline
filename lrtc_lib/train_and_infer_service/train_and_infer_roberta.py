...
from transformers import RobertaConfig, RobertaTokenizer, AutoTokenizer, TFRobertaForSequenceClassification, TFBertForSequenceClassification, InputFeatures
from tensorflow.python.keras.mixed_precision.experimental import (loss_scale_optimizer as lso)
from tensorflow.keras import backend as K

from  definitions import ROOT_DIR
from  train_and_infer_service.train_and_infer_api import TrainAndInferAPI, ModelStatus, infer_with_cache

MODEL_DIR = "/home/mamooler/models"#os.path.join(ROOT_DIR, "output", "models", "transformers")
HF_CACHE_DIR = "/home/mamooler/temp"#os.path.join(ROOT_DIR, "output", "temp", "hf_cache")


class TrainAndInferRoberta(TrainAndInferAPI):
    def __init__(self, batch_size, checkpoint_path, infer_batch_size=10, learning_rate=5e-5, debug=False, model_dir=MODEL_DIR,
                 infer_with_cls=True):
        """
        :param batch_size:
        ...
        classifier layer.
        """
        super(TrainAndInferRoberta, self).__init__()
    ...
    def get_tokenizer(self):
        return AutoTokenizer.from_pretrained(self.ckpt_path, cache_dir=HF_CACHE_DIR)
    ...
    def train(self, train_data, dev_data, test_data, train_params: dict) -> str:
        ...
        try:
            ...
            # init
            with strategy.scope():
                if self.ckpt_path=="roberta-base":
                    model = TFRobertaForSequenceClassification.from_pretrained(self.ckpt_path, cache_dir=HF_CACHE_DIR)
                elif self.ckpt_path=="nlpaueb/legal-bert-base-uncased":
                    model = TFBertForSequenceClassification.from_pretrained(self.ckpt_path, cache_dir=HF_CACHE_DIR)
                else:
                    model = TFRobertaForSequenceClassification.from_pretrained(self.ckpt_path, cache_dir=HF_CACHE_DIR, from_pt=True)
            ...
            class_weight = {0 : (1/labels.count(0))*(len(labels)/2), 1: (1/labels.count(1))*(len(labels)/2)}
            ...
            early_stopping = EarlyStopping(monitor="val_" + metric_name, patience=epochs//10)
            ...
            if os.path.isfile(train_file):
                ...
                logging.info("removed files no longer needed.")
    ...
    def infer(self, model_id, items_to_infer, infer_params: dict, use_cache=True):
        ...
        #if os.path.exists(self.get_model_dir_by_id(model_id)):
        if self.ckpt_path=="nlpaueb/legal-bert-base-uncased":
            model = TFBertForSequenceClassification.from_pretrained(self.get_model_dir_by_id(model_id))
        else: #if self.ckpt_path=="roberta-base":
            model = TFRobertaForSequenceClassification.from_pretrained(self.get_model_dir_by_id(model_id))
        # else:
        #     model = TFRobertaForSequenceClassification.from_pretrained(self.get_model_dir_by_id(model_id), from_pt=True)
        ...
        if self.infer_with_cls:  # get embeddings for CLS token in last hidden layer
            ...
            outputs = [model.roberta(inp[0])[1] for inp in input]
