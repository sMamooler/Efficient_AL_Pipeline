...
import  data_access.core.data_access_in_memory_logic as logic
import  data_access.core.utils as utils
from  data_access.core.data_access_in_memory_logic import LabeledStatus
from  data_access.core.data_structs import Document, Label, TextElement
...
class DataAccessInMemory(DataAccessApi):
    ...
    def remove_from_data(self, workspace_id: str, uris: Sequence[str]):
        """
        removes the texts with the provided uris from the data
        :param workspace_id: the workspace_id of the labeling effort.
        :param uris: uris of the texts to remove
        """
        ...
        for uri in uris:
            if uri not in all_uris:
                raise Exception(f'Trying to remove the text with uri "{uri}" which does not exist')

            print(logic.labels_in_memory[workspace_id][dataset_name])
            if uri in logic.labels_in_memory[workspace_id][dataset_name].keys():
                print("##############################################")
                logic.labels_in_memory[workspace_id][dataset_name].pop(uri)
