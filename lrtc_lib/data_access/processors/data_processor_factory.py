...
from  data_access.processors.process_ag_new_data import AgNewsProcessor
from  data_access.processors.process_cola_data import ColaProcessor
from  data_access.processors.process_isear_data import IsearProcessor
from  data_access.processors.process_polarity_data import PolarityProcessor
from  data_access.processors.process_trec_data import TrecProcessor
from  data_access.processors.process_wiki_attack_data import WikiAttackProcessor
...
from  data_access.processors.process_subjectivity_data import SubjectivityProcessor
from  data_access.processors.process_contract_nli_data import ContractNLIProcessor
from  data_access.processors.process_opp_data import OPPProcessor
from  data_access.processors.process_ledgar_data import LEDGARProcessor
...
def get_data_processor(dataset_name: str) -> DataProcessorAPI:
    ...
    if dataset_source == 'contract_nli':
        return ContractNLIProcessor(dataset_part=dataset_part)
    if dataset_source == 'opp':
        return OPPProcessor(dataset_part=dataset_part)
    if dataset_source == 'ledgar':
        return LEDGARProcessor(dataset_part=dataset_part)
    else:
