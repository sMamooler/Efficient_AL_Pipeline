...
class PerceptronEnsemble(ActiveLearner):
    ...
    def get_recommended_items_for_labeling(self, workspace_id, model_id, checkpoint_path, dataset_name, category_name, view_dir="", medoids_only=False, sample_size=1):
        ...
        scores = self.get_per_element_score(unlabeled, workspace_id, model_id, checkpoint_path, dataset_name, category_name)
        selected_idx = np.argpartition(scores, -sample_size)[-sample_size:]
        res = np.array(unlabeled)[selected_idx]
        self.nb_actions = len(res)
        return res.tolist()
    ...
    def get_per_element_score(self, items, workspace_id, model_id, checkpoint_path, dataset_name, category_name):
        ...
        train_embeddings = np.array(orchestrator_api.infer(workspace_id, category_name, checkpoint_path, labeled)["embeddings"])
        unlabeled_embeddings = np.array(orchestrator_api.infer(workspace_id, category_name, checkpoint_path, items)["embeddings"])
...
def get_pos_idx(category_name, labeled):
    from  orchestrator.orchestrator_api import LABEL_POSITIVE
